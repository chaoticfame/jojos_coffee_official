...
if (isset($_SESSION['user_id'])) {
    if ($_SESSION['user_role'] === 'admin') {
        ...
        // Regular users go straight to the main menu page
...
// Check if persistent cart table exists
$cartTableExists = false;
$tableCheckCart = $conn->query("SHOW TABLES LIKE 'user_cart_items'");
if ($tableCheckCart && $tableCheckCart->num_rows > 0) {
            ...
            if ($stmt) {
                ...
                if ($result->num_rows === 1) {
                    ...
                    if (password_verify($password, $user['password'])) {
                        // Determine role (admin vs regular user)
                        $userRole = $user['role'] ?? 'user';

                        // Preserve any cart items from this session before login
                        $sessionCartBeforeLogin = $_SESSION['cart'] ?? [];
                        ...
                        // If we have a persistent cart table, merge guest cart → DB cart and hydrate session
                        if ($cartTableExists) {
                            $userId = (int)$user['id'];
                            $dbCart = [];

                            // Load existing DB cart
                            if ($stmtCart = $conn->prepare("SELECT menu_item_id, qty FROM user_cart_items WHERE user_id = ?")) {
                                $stmtCart->bind_param('i', $userId);
                                $stmtCart->execute();
                                $resCart = $stmtCart->get_result();
                                while ($rowCart = $resCart->fetch_assoc()) {
                                    $dbCart[(int)$rowCart['menu_item_id']] = (int)$rowCart['qty'];
                                }
                                $stmtCart->close();
                            }

                            // Merge any guest cart items into DB cart (add quantities)
                            if (!empty($sessionCartBeforeLogin)) {
                                foreach ($sessionCartBeforeLogin as $mid => $qtyGuest) {
                                    $mid = (int)$mid;
                                    $qtyGuest = (int)$qtyGuest;
                                    if ($qtyGuest <= 0) {
                                        continue;
                                    }
                                    if (!isset($dbCart[$mid])) {
                                ...
                                // Replace DB rows with merged cart
                                if (!empty($dbCart)) {
                                    ...
                                    if ($insertStmt = $conn->prepare("INSERT INTO user_cart_items (user_id, menu_item_id, qty) VALUES (?, ?, ?)")) {
                                        foreach ($dbCart as $mid => $qty) {
                                            $insertStmt->bind_param('iii', $userId, $mid, $qty);
                                            $insertStmt->execute();
                                        }
                                        $insertStmt->close();
                                    }
                            ...
                            // Hydrate session cart from DB cart
                        ...
                        if ($userRole === 'admin') {
                            ...
                            // After successful login, regular users land on the main menu
                            header('Location: menu.php');
                        }
                        exit();
                    } else {
...
<head>
  ...
  <title>JoJo's Bizarre Café — Login</title>
...
<body class="login-page">
  ...
  <main class="container login-container">
    <div class="login-card">
      <h2>Sign In</h2>
      <p style="color: #666; font-size: 14px; margin-bottom: 20px;">Regular users Sign In here.</p>
